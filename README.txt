...
- RVOL BADGE (TradingView only): an optional tiny badge next to each symbol's
  name showing its RVOL as a %. Color reads volume AND direction: green =
  heavy volume (a full average day already traded) and the stock is up, red =
  heavy volume and down, amber = heavy volume but barely moving, grey = light
  volume. It works on its own, no sort needs to be on.
